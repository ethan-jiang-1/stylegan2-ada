...
def get_custom_options():
    cwd = os.getcwd()
    copts = {}
    copts["outdir"] = "{}/styleganada-results".format(cwd)
    copts["gpus"] = None
    copts["data"] = "{}/data_planets/custom".format(cwd)
    copts["resume"] = "{}/data_planets/resume/network-snapshot-00030.pkl".format(cwd)
    return copts
...
def setup_training_options():
    ...
    copts = get_custom_options()
    
    #outdir = '/content/drive/MyDrive/styleganada-results/'
    outdir = copts["outdir"]
...
def run_training():
    ...
    if SKIP_TRANING:
        print("set resume_time to -1")
        training_options.resume_time = -1.0
    ...
    json_path = os.path.join(training_options.run_dir, 'training_options.json')
    ...
    log_path = os.path.join(training_options.run_dir, 'log.txt')
    print("*log_path", log_path)    
    with dnnlib.util.Logger(log_path):
        print(training_options)
        grid_latents, last_pkl = training_loop_modified.training_loop(**training_options)

    return grid_latents, last_pkl
...
if __name__ == "__main__":
    copts = get_custom_options()
    grid_latents, last_pkl = run_training()
    #main()
