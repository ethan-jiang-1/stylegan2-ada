...
class InterruptSignal(object):
    ...
    @classmethod
    def has_received_stop_signal(cls):
...
def setup_snapshot_image_grid(training_set):
    ...
    gw = np.clip((256*10) // training_set.shape[2], 7, 32)
    gh = np.clip((256*10) // training_set.shape[1], 4, 32)
...
def training_loop(
    ...
    print("wait for see if we have kill signal")
    if resume_time < 0:
        print("resume_time < 0, abort training")
        return grid_latents, None
    ...
    while not done:
        ...
        for _repeat_idx in range(minibatch_repeats):
            ...
            if InterruptSignal.has_received_stop_signal():
                break

        # Tune augmentation parameters.
        if aug is not None:
            if not InterruptSignal.has_received_stop_signal():
